...
if (isset($_REQUEST["mode"])) {
    ...
    if ($mode == 'login') {
        ...
        // phone: 
        // password: 
        if (isset($_REQUEST["email"]) && isset($_REQUEST["phone"]) && isset($_REQUEST["password"]) && isset($_REQUEST["name"])) {
            try {
                $email = trim(htmlspecialchars($_REQUEST["email"]));
                $password = trim(htmlspecialchars($_REQUEST["password"]));
            ...
            if (isset($_SESSION['id']) && isset($_SESSION['email'])) {
                ...
                try {
                    ...
                    if ($userAuth) {
                        ...
                        $sql = "INSERT INTO class (status, title, category, content, author, created_at, updated_at,images) VALUES (:status, :title, :category, :content, :author, :created_at, :updated_at, :images)";
                        $stmt = $conn->prepare($sql);
            ...
            if (isset($_SESSION['id']) && isset($_SESSION['email'])) {
                ...
                try {
                    ...
                    if ($userAuth) {
                        ...
                        $sql = "UPDATE class SET status = :status, title = :title, category = :category, content = :content, author = :author, updated_at = :updated_at, images = :images  WHERE id = :id";
                        $stmt = $conn->prepare($sql);
                        $stmt->bindParam(":status", $status);
                        $stmt->bindParam(":title", $name);
                        $stmt->bindParam(":category", $category);
